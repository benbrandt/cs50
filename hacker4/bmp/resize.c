    ...
    for (int i = 0, biHeight = abs(in_bi.biHeight); i < biHeight; i++)
    {
        ...
        for (int k = 0; k < in_bi.biWidth; k++)
        {
            ...
            if (n > 1) 
            {
                ...
                for (int m = 0; m < ceil(n) && current_pixel < out_bi.biWidth; m++, current_pixel++)
                {
                    row[current_pixel] = triple;
                }
            }
            else {
                if (k % (int) (1 / n) == 0 && current_pixel < out_bi.biWidth)
                {
                    row[current_pixel] = triple;
                    current_pixel++;
                }
        ...
        if (n > 1) 
        {
            ...
            for (int j = 0; j < ceil(n) && current_row < abs(out_bi.biHeight); j++, current_row++)
            {
        ...
        {
            if (i % (int) (1 / n) == 0 && current_row < abs(out_bi.biHeight)) 
            {
                // Write row to out file
                fwrite(&row, sizeof(row), 1, outptr);
